...
`SERIALx_PROTOCOL=51` selects the `MotorControlOutput` protocol. When that protocol is selected, ArduPilot defaults the port to 921600 baud, disables flow control, and uses unbuffered writes. Setting `SERIALx_BAUD=921` makes that baud rate explicit in the parameter file.
...
To adjust either trim from a transmitter knob or slider, assign that RC channel `RCx_OPTION=219` (`Transmitter Tuning`), set `TUNE=62` for roll or `TUNE=63` for pitch, and choose the knob endpoints with `TUNE_MIN` and `TUNE_MAX`. Select and tune one axis at a time.
...
If the receiver prints `waiting for valid MCOUT packets`, check that the selected ArduPilot serial port has `SERIALx_PROTOCOL=51`, the baud rates match, grounds are connected, and the ArduPilot TX pin is wired to the receiver RX pin.
